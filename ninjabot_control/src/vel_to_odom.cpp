...
#include <stdio.h>
#include <cmath>
#include <ninjabot_msgs/Wheel_speed.h>

double radius = 0.05325;                              //Wheel radius, in m
double wheelbase = 0.274;                          //Wheelbase, in m
...
void handle_right_speed( const ninjabot_msgs::Wheel_speed& speed) {
  speed_act_right = trunc(speed.speed*100)/100;
  ROS_INFO("speed right : %f", speed_act_right);
}
void handle_left_speed( const ninjabot_msgs::Wheel_speed& speed) {
  speed_act_left = trunc(speed.speed*100)/100;
  ROS_INFO("speed left : %f", speed_act_left);
  speed_dt = speed.loop_time;
...
int main(int argc, char** argv){
  ...
  ros::Subscriber leftWheelSpeedsub = n.subscribe("ninjabot/leftWheelSpeed", 40, handle_left_speed);
  ros::Subscriber rightWheelSpeedsub = n.subscribe("ninjabot/rightWheelSpeed", 40, handle_right_speed);
  ros::Publisher odom_pub = n.advertise<nav_msgs::Odometry>("/ninjabot/odom_enc", 30);
